...
class Analysis:
    ...
    def problem_analysis(self):
        ...
        subject_prob_total = self.subject_prob_total
        ...
        if "客观题" in self.item_score.values:
            ...
            for sub in range(0, sub_num):
                ...
                # 难度计算
                high_group_ave = high_group[prob_name].mean()
                low_group_ave = low_group[prob_name].mean()
        ...
        if "主观题" in self.item_score.values:
            ...
            for obj in range(first_obj, prob_num):
                ...
                low_group_ave = low_group[prob_name].mean()
                group_ave = (high_group_ave + low_group_ave) / 2
                obj_difficulty = np.round(group_ave / highest, 2)
        ...
        if "预估难度" in list(subject_prob_total.columns):
    ...
    def summary_analysis(self, test_difficulty, prob_analysis_distinction, subject_total_score,
        ...
        summary_dict = {"科目": [self.subject_name], "分值": [subject_full_score], "预估难度": [np.nan],
                        ...
                        "得分率": test_score_rate, "单科与总分相关系数": np.round(corr_list[subject_index], 4)}
